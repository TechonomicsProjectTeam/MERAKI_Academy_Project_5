...
const addProductToCart = (req, res) => {
  ...
  pool
    .query(`SELECT * FROM cart WHERE cart_id = $1 AND user_id = $2`, [
      cart_id,
      user_id,
    ])
    .then((result) => {
          ...
          return pool
            ...
            .then((cartProductResult) => {
              if (cartProductResult.rows.length > 0) {
                ...
                return pool
                  .query(
                    `UPDATE cart_products SET quantity = quantity + $1 WHERE cart_id = $2 AND product_id = $3 RETURNING *`,
...
const getAllCart = (req, res) => {
  ...
  pool
    .query(
      `SELECT * FROM cart WHERE cart_id = $1 AND user_id = $2 AND is_deleted = 0`,
      [cart_id, user_id]
    )
    .then((result) => {
      if (result.rows.length === 0) {
      ...
      pool
        .query(
          `SELECT cart_products.* ,products.name , products.price ,products.images
  ...
  pool
    ...
    .then((result) => {
      ...
      if (currentQuantity > 1) {
        return pool
          .query(
            `UPDATE cart_products SET quantity = quantity - 1 WHERE cart_id = $1 AND product_id = $2 RETURNING *`,
            [cart_id, product_id]
          )
          .then((updateResult) => {
            res.status(200).json({
...
const getCartProductsByUserId = (req, res) => {
  const { userId } = req.token;

  pool.query(`SELECT cart_id FROM cart WHERE is_deleted = 0 AND user_id = $1`, [userId])
    .then((result) => {
      if (result.rows.length === 0) {
        return res.status(404).json({
          success: false,
          message: `No active cart found for user_id ${userId}`,
        });
      }
      ...
      pool.query(
        ...
        res.status(200).json({
          success: true,
          message: `All products in the cart for user_id ${userId}`,
