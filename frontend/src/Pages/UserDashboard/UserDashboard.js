...
import { setReviewsByProduct, addReview, updateReview, deleteReview } from '../../redux/reducers/Reviews/Reviews';
...
const UserDashboard = () => {
  ...
  const fetchReviews = async (product_id) => {
    try {
      const result = await axios.get(`http://localhost:5000/review/${product_id}`);
      if (result.data.success) {
        dispatch(setReviewsByProduct({
          product_id,
  ...
  const handleAddReview = async (product_id) => {
    ...
    try {
      const result = await axios.post(`http://localhost:5000/review/${product_id}`, newReview, {
        headers: {
          Authorization: `Bearer ${token}`,
        }
      });
      if (result.data.success) {
        ...
        setNewReviews({ ...newReviews, [product_id]: { rating: 0, review_text: "" } });
      }
    } catch (error) {
  ...
  const handleUpdateReview = async (review_id, product_id) => {
    try {
      const result = await axios.put(`http://localhost:5000/review/${review_id}`, editReviewData, {
        headers: {
          Authorization: `Bearer ${token}`,
        }
      });
  ...
  const handleDeleteReview = async (review_id, product_id) => {
    try {
      const result = await axios.delete(`http://localhost:5000/review/${review_id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        }
      });
    ...
    axios.get(`http://localhost:5000/product/${shopId}`)
      .then(response => {
        if (response.data.success) {
          ...
          axios.get(`http://localhost:5000/shop/${shopId}`)
            .then(response => {
              if (response.data.success) {
    ...
    <ul className='product-list'>
      {displayedProducts.map(product => (
        <li key={product.product_id}>
          ...
          <div>
            {reviews[product.product_id] && reviews[product.product_id].map(review => (
              <div key={review.review_id}>
                <p>{review.review_text} - {review.rating} stars</p>
                {editingReview === review.review_id ? (
                  <div>
                    <div>
                      {[1, 2, 3, 4, 5].map(star => (
                        <span 
                          ...
                          style={{ cursor: 'pointer', color: star <= editReviewData.rating ? '#ffc107' : '#e4e5e9' }}
                        >
                          ★
                        </span>
                      ))}
                    ...
                    <input 
                      ...
                      onChange={(e) => handleEditReviewChange('review_text', e.target.value)}
                    />
                    <button onClick={() => handleUpdateReview(review.review_id, product.product_id)}>Submit Update</button>
                  </div>
                ) : (
                  <div>
                    <button onClick={() => setEditingReview(review.review_id)}>Update</button>
                    <button onClick={() => handleDeleteReview(review.review_id, product.product_id)}>Delete</button>
                  </div>
                )}
              </div>
            ))}
            <div>
              ...
              <div>
                {[1, 2, 3, 4, 5].map(star => (
                  <span 
                    key={star} 
                    onClick={() => handleStarClick(product.product_id, star)}
                    style={{ cursor: 'pointer', color: star <= (newReviews[product.product_id]?.rating || 0) ? '#ffc107' : '#e4e5e9' }}
                  >
                    ★
                  </span>
                ))}
              ...
              <input 
                ...
                onChange={(e) => setNewReviews({
                  ...newReviews,
                  [product.product_id]: { ...newReviews[product.product_id], review_text: e.target.value }
                })}
              />
              <button onClick={() => handleAddReview(product.product_id)}>Submit Review</button>
            </div>
          </div>
        </li>
      ))}
